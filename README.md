...
The project uses **unit tests** to test business logic in isolation. Tests are located in `backend/tests/unit/` directory.

#### Unit Tests

Unit tests test individual functions or components in isolation, without dependencies.

**What is tested:**
- Service layer functions (business logic)
- Validation logic
- Business rules (discount calculation, status assignment)

**What is mocked:**
- Repository layer (database queries are mocked)
...
- Product listing with status and discount price calculation
- Product creation with validation
- Product deletion

**Example test:**
```javascript
// Tests that products get correct status and discount price
it('should return products with status and discountPrice', async () => {
  ...
  expect(result[0].discountPrice).toBe('17.00');
});
```

#### CartService Tests

Tests for cart business logic including:
- Cart creation
- Getting cart with products
- Adding products to cart with validation
- Error handling for invalid inputs

**Example test:**
```javascript
// Tests validation when adding product to cart
it('should throw error when quantity is zero or negative', async () => {
  await expect(addProduct(1, 1, 0)).rejects.toThrow(
    'Kogus peab olema suurem kui 0'
  );
});
```
...
**Note:** Integration tests (testing Controller → Service → Repository flows) are planned for future implementation.
